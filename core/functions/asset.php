...
function asset_data($con,$id){
        $data =array();
        $id= (int)$id;
        $res=mysqli_query($con,"SELECT `sno`,`user`,`department`,`machinetype`,`manufacturer`,`machinesn`,`processor`,`os`,`systemspecs`,`dateofpurchase`,`warrantystatus`,`vendor`,`fromyear`,`totalyear`,`tyear` FROM `assets` WHERE `id`= $id");
